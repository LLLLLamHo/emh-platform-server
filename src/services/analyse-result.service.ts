import Koa from 'koa';
import { AnalyseResultCreationAttributes } from '../db/analyse-result';
...
class AnalyseResultService {
  ...
  async saveAnalyseResult(ctx: Koa.Context, params: AnalyseResultCreationAttributes) {
    try {
      const { userId, year, month, status } = params;

      const [result, created] = await ctx.state.db.analyseResultModule.upsert({
        userId,
        year,
        month,
        status,
      ...
      return { error: null, result: { created, result } };
    } catch (error: any) {
